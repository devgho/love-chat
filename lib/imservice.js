...
//获取群成员
IMService.prototype.getGroupMembers = async function (groupId) {
    let result = await restApi.findGroupMembers(groupId);
	let members = result.members;
	members.push(result.master)
    let membersMap = {};
    members.map(item => {
...
IMService.prototype.findGroupById = async function (groupId) {
    let group = await restApi.findGroupById(groupId);
	let ngroup = new Group(group.uuid, group.name, group.avatar);
    return ngroup;
};
...
IMService.prototype.subscribeGroupMessage = async function () {
    let groups = await restApi.findGroups(this.currentUser);
