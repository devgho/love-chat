<template>
	<view class="chatInterface">
		<uni-nav-bar left-icon="back" @clickLeft="back()" @clickRight="follow()" left-text="返回"
			:right-text="is_follow? '取关':'关注'" :title="this.friend.name"></uni-nav-bar>
	...
	export default {
		...
		methods: {
			...
			has_follow() {
				...
				const follow_tb = db.collection("uni-id-followers").where({
					friend_uid: this.friend.uuid,
					follow_uid: this.currentUser.uuid
				}).get().then(res => {
			...
			follow() {
				if (this.is_follow) {
					if (confirm("是否取关？")) {
						const db = uniCloud.database()
						const follow_tb = db.collection("uni-id-followers")
						follow_tb.where({
							"follow_uid": this.currentUser.uuid,
							"friend_uid": this.friend.uuid
						}).remove()
